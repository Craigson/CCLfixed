...
std::vector<CCL_MocapJoint> loadMotionCaptureFromSite(const ci::Url &url, int skip)
{
  ...
  try{
     ...
     //         CCL_MocapData::addUUIDtoJoint(group, title, uuid, skip, mJoints);
          }
      }
...
CCL_MocapData::CCL_MocapData(int skip, std::vector<CCL_MocapJoint>& mJoints){
    try{
        ...
        root.Add("joints", joints);
        cout << "Start writing file as CCL_JOINT_CCL4_04.json" <<endl;
        Jzon::FileWriter::WriteFile("CCL_JOINT_CCL4_04.json", root, Jzon::StandardFormat);
        cout << "Ended writing file" <<endl;
